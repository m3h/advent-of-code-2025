...
(while remaining_moves
  ...
  (while (> current_rotations 0)
    (when (string-equal current_direction "L")
      (setq current_position (- current_position 1))      
      )
    (when (string-equal current_direction "R")
      (setq current_position (+ current_position 1))
      )
    (setq current_rotations (- current_rotations 1))

    (when (= current_position 100) (setq current_position 0))
    (when (= current_position -1) (setq current_position 99))
